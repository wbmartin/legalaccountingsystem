 ...
 *
 */
public class InvoiceController implements AppEventListener, ClickHandler, ChangeHandler{
  ...
  private final InvoiceServiceAsync invoiceService = 
	GWT.create(InvoiceService.class); 		// primary GWT remote Service
 ...
 */
  public static InvoiceController getInstance(MasterController masterController_){
	if(instance == null){
 ...
 */
 private void selectInvoiceBeans( final String whereClause_, final String orderByClause_){
		...
		invoiceService.selectInvoice(userProfile, whereClause_, orderByClause_, 
				new AsyncCallback<ArrayList<InvoiceBean>>(){
					...
					public void onSuccess(ArrayList<InvoiceBean> invoiceResult) {
						masterController.getAppContainer().addSysLogMessage("Select Invoice received ok-  Where Attempted: " 
 ...
 * @param orderByClause a string beginning with "order by" using standard sql syntax appropriate alter the order of the beans
 */
 private void retrieveInvoiceBeanByPrKey( Integer invoiceId_ , Integer customerId_ ){
	final java.util.Date startTime = new java.util.Date();
		invoiceService.getInvoiceByPrKey(userProfile,  invoiceId_ , customerId_ , 
				new AsyncCallback<InvoiceBean>(){
					...
					public void onSuccess(InvoiceBean invoiceResult) {
						masterController.getAppContainer().addSysLogMessage("Retrieve Invoice by identifier received ok");
						masterController.getAppContainer().setTransactionResults(
 ...
 * The results are handled through the onSuccess method in the AsynchCallback.
 * @param invoiceBean_ the bean to insert into the database
 */
  private void insertInvoiceBean(final InvoiceBean invoiceBean_){
	...
	invoiceService.insertInvoiceBean(userProfile,invoiceBean_,
			new AsyncCallback<InvoiceBean>(){
				public void onFailure(Throwable caught) {
					Log.debug("invoiceService.insertInvoice Failed: " + caught);
					masterController.notifyUserOfSystemError("Remote Procedure Call - Failure", 
							AppPref.SERVER_ERROR + caught.getMessage());
					masterController.getAppContainer().setTransactionResults(
						"Adding Invoice Failed"
						, (new java.util.Date().getTime() -startTime.getTime()));
					masterController.getAppContainer().addSysLogMessage("Insert Bean Attempted: " + invoiceBean_);

				}
	
				public void onSuccess(InvoiceBean result) {
					Log.debug("invoiceService.insertInvoice onSuccess: " + result);
					if (  result.getInvoiceId() !=null && result.getClientId() !=null && result.getCustomerId() !=null){
 ...
 * The results are handled through the onSuccess method in the AsynchCallback.
 * @param invoiceBean_ the bean to update the database
 */
   private void updateInvoiceBean(final InvoiceBean invoiceBean_){
	...
	invoiceService.updateInvoiceBean(userProfile,invoiceBean_,
			new AsyncCallback<InvoiceBean>(){
				public void onFailure(Throwable caught) {
					Log.debug("invoiceService.updateInvoice Failed: " + caught);
					masterController.notifyUserOfSystemError("Remote Procedure Call - Failure", 
							AppPref.SERVER_ERROR + caught.getMessage());
					masterController.getAppContainer().setTransactionResults(
						"Updating Invoice Failed"
						, (new java.util.Date().getTime() -startTime.getTime()));
					masterController.getAppContainer().addSysLogMessage("Update Bean Attempted: " + invoiceBean_);

				}
	
				public void onSuccess(InvoiceBean result) {
					Log.debug("invoiceService.updateInvoice onSuccess: " + result);
					if (  result.getInvoiceId() !=null && result.getClientId() !=null && result.getCustomerId() !=null){
						userProfile.incrementSessionTimeOut();
						masterController.getAppContainer().setTransactionResults(
 ...
 * The results are handled through the onSuccess method in the AsynchCallback.
 * @param invoiceBean_ the bean to delete the database
 */
			...
			new AsyncCallback<Boolean>(){
				public void onFailure(Throwable caught) {
					...
					masterController.getAppContainer().setTransactionResults(
						...
						, (new java.util.Date().getTime() -startTime.getTime()));
					masterController.getAppContainer().addSysLogMessage("Delete Bean Attempted: " + invoiceBean_);

				}
	

				public void onSuccess(Boolean result) {
					Log.debug("invoiceService.deleteInvoice onSuccess: " + result);
					if (result){
						userProfile.incrementSessionTimeOut();
						masterController.getAppContainer().setTransactionResults(
							"Successfully deleted Invoice record"
							, (new java.util.Date().getTime() - startTime.getTime()));
						masterController.getAppContainer().addSysLogMessage("Bean Deleted" +  invoiceBean_.toString());
					}else{
 ...
 * The results are handled through the onSuccess method in the AsynchCallback.
 * @param invoiceBean_ the bean to save to the database
 */
  private void saveInvoiceBean(final InvoiceBean invoiceBean_){
	...
	invoiceService.saveInvoiceBean(userProfile,invoiceBean_,
			new AsyncCallback<InvoiceBean>(){
				public void onFailure(Throwable caught) {
					Log.debug("invoiceService.saveInvoice Failed: " + caught);
					masterController.notifyUserOfSystemError("Remote Procedure Call - Failure", 
							AppPref.SERVER_ERROR + caught.getMessage());
					masterController.getAppContainer().setTransactionResults(
						"Saving Invoice Failed"
						, (new java.util.Date().getTime() -startTime.getTime()));
					masterController.getAppContainer().addSysLogMessage("Save Bean Attempted: " + invoiceBean_);

				}
	
				public void onSuccess(InvoiceBean result) {
					userProfile.incrementSessionTimeOut();
					Log.debug("invoiceService.saveInvoice onSuccess: " + result);
					if (  result.getInvoiceId() !=null && result.getClientId() !=null && result.getCustomerId() !=null){
						masterController.getAppContainer().setTransactionResults(
							"Successfully saved Invoice record"
							, (new java.util.Date().getTime() - startTime.getTime()));
						masterController.getAppContainer().addSysLogMessage("Bean Saved" +  invoiceBean_.toString());
					}else{
 ...
 */
  private void saveInvoiceBeanBatch(ArrayList<InvoiceBean> invoiceBeanList_){
	...
	invoiceService.saveInvoiceBeanBatch(userProfile, invoiceBeanList_, 
			new AsyncCallback<ArrayList<InvoiceBean>>(){
				public void onFailure(Throwable caught) {
					Log.debug("invoiceService.saveInvoiceBeanBatch Failed: " + caught);
					masterController.notifyUserOfSystemError("Remote Procedure Call - Failure", 
							AppPref.SERVER_ERROR + caught.getMessage());
					masterController.getAppContainer().setTransactionResults(
				...
				public void onSuccess(ArrayList<InvoiceBean> invoiceResult) {
					Log.debug("invoiceService.saveInvoiceBeanBatch onSuccess: " + invoiceResult.toString());
					masterController.getAppContainer().setTransactionResults(
